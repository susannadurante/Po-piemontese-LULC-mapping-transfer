# Seasonal median Sentinel-2 L2A composite - autumn (SON, Sep-Nov 2019) - WHOLE PARK
...
dir_s2     <- file.path(DIR_BASE, "S2", "SON")   # Sentinel-2 archives for the season
...
file_collection <- file.path(DIR_BASE, "output_spettrale", "S2_SON_4tile_collection.db")
...
.f_vc <- file.path(DIR_BASE, "output_spettrale",
                   "S2_MSIL2A_SON2019_T32TLQ-32TLR-32TMQ-32TMR_mediana_10bande.tif")
...
vista <- cube_view(
  srs        = "EPSG:32632",
  extent     = list(
    t0     = "2019-09-01",         # season start
    t1     = "2019-11-30",         # season end
...
# Build the composite ----
dir_temp <- file.path(DIR_BASE, "tmp_S2_SON_intero_parco")
...
# expected: mean B08 (NIR) > mean B04 (red) for autumn vegetation
