# Seasonal median Sentinel-2 L2A composite - spring (MAM, Mar-May 2019) - WHOLE PARK
...
dir_s2     <- file.path(DIR_BASE, "S2", "MAM")   # Sentinel-2 archives for the season
...
file_collection <- file.path(DIR_BASE, "output_spettrale", "S2_MAM_4tile_collection.db")
...
vista <- cube_view(
  srs        = "EPSG:32632",
  extent     = list(
    t0     = "2019-03-01",         # season start
    t1     = "2019-05-31",         # season end
...
# Build the composite ----
dir_temp <- file.path(DIR_BASE, "tmp_S2_MAM_intero_parco")
...
# expected: mean B08 (NIR) > mean B04 (red) for spring vegetation
